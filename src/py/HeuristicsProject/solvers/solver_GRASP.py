...
class Solver_GRASP(_Solver):
    ...
    def _selectAssignmentCandidate(self, candidateList, alpha, solution):
        ...
        lowestRatedAssignment = sortedCandidateList[-1]
        highestRatedAssignment = sortedCandidateList[0]
        minRating = solution.getRating(lowestRatedAssignment.orderId, lowestRatedAssignment.start)
        maxRating = solution.getRating(highestRatedAssignment.orderId, highestRatedAssignment.start)
        ...
        for candidate in sortedCandidateList:
            if solution.getRating(candidate.orderId, candidate.start) >= boundaryScore:
                maxIndex += 1
    ...
    def _greedyRandomizedConstruction(self, alpha):
        ...
        # for i in range(0, len(sortedOrders)):
            ...
            candidate = self._selectAssignmentCandidate(candidateList, alpha, solution)
            if not candidate:
